...
const useStyles = makeStyles({
  ...
  paper: {
    background: 'linear-gradient(33deg, rgb(75 75 196) 20%, rgb(0 166 200) 90%)',
    height: '8vh',
    width: '100%',
    padding: '0.5rem',
    borderRadius: '1rem',
      ...
      <Paper className={classes.paper} elevation={5}>
        ...
        <div className={classes.listState}>
          <div className={classes.stateBox}>
            <LgTypography className={classes.state}>{crtJob.state}</LgTypography>
          </div>
        </div>
        <div className={classes.listControl}>
          <SettingsIcon className={classes.icon} onClick={() => handleClickOpenEdit(crtJob)} />
        </div>
      </Paper>
    </div>
  );
